...
| gcpn6_intro (IN: cpn6 gf (r \6/ fcpn6 gf rg) e0 e1 e2 e3 e4 e5)
.              
...
Lemma gcpn6_inc_mon r rg:
  monotone6 (fun x : rel => gcpn6 r (rg \6/ x)).
Proof.
  red; intros.
...
Lemma gcpn6_init r: gcpn6 r r <6= cpn6 gf r.
Proof.
  intros. destruct PR.
  ucpn.
...
Lemma gcpn6_final r rg: cpn6 gf r <6= gcpn6 r rg.
Proof.
...
Lemma gcpn6_cpn r rg:
  cpn6 gf (gcpn6 r rg) <6= gcpn6 r rg.
Proof.
...
Lemma cut6_wcomp r rg (LE: r <6= rg) :
  wcompatible6 gf (cut6 (cpn6 (fun x => gcpn6 r (rg \6/ x)) bot6) rg).
Proof.
  set (pfix := cpn6 (fun x => gcpn6 r (rg \6/ x)) bot6).
  
  econstructor; [apply cut6_mon|]. intros.
  destruct PR as [LEz FIX].
  uunfold FIX; [|apply gcpn6_inc_mon].
...
Lemma fix6_le_cpn r rg (LE: r <6= rg) :
  cpn6 (fun x => gcpn6 r (rg \6/ x)) bot6 <6= cpn6 gf rg.
Proof.
  intros. eexists.
...
Lemma fix6_le_gcpn r rg (LE: r <6= rg):
  cpn6 (fun x => gcpn6 r (rg \6/ x)) bot6 <6= gcpn6 r rg.
Proof.
  (*
    fix
    =
  ...
  intros. uunfold PR; [| apply gcpn6_inc_mon].
...
Lemma gcpn6_cofix: forall
  ...
  l <6= gcpn6 r rg.
Proof.
  intros. apply fix6_le_gcpn. apply LE.
  eapply cpn6_algebra, PR. apply gcpn6_inc_mon.
...
Lemma gcpn6_mon_bot (gf gf': rel -> rel) e0 e1 e2 e3 e4 e5 r rg
      (IN: @gcpn6 gf bot6 bot6 e0 e1 e2 e3 e4 e5)
      (MONgf: monotone6 gf)
      (MONgf': monotone6 gf')
      (LE: gf <7= gf'):
  @gcpn6 gf' r rg e0 e1 e2 e3 e4 e5.
Proof.
  destruct IN. constructor.
  eapply cpn6_mon; [| intros; right; eapply PR].
  ubase.
  eapply fcpn6_mon_bot, LE; [|apply MONgf|apply MONgf'].
