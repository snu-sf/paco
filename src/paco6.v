...
Theorem paco6_acc r
      (X: Type) x0 x1 x2 x3 x4 x5
      (OBG: forall rr (INC: r <6= rr) (CIH: forall (x: X), rr (x0 x) (x1 x) (x2 x) (x3 x) (x4 x) (x5 x)),
            forall (x: X), paco6 gf rr (x0 x) (x1 x) (x2 x) (x3 x) (x4 x) (x5 x)):
  forall (x: X), paco6 gf r (x0 x) (x1 x) (x2 x) (x3 x) (x4 x) (x5 x).
Proof.
  eapply paco_acc.
  intros. apply uncurry_adjoint1_6 in INC.
  eapply paco_mon.
  - apply (OBG (curry6 rr) INC CIH x).
  - intros. destruct x6. assumption.
Qed.
