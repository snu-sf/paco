...
Theorem paco1_acc r
      (X: Type) x0
      (OBG: forall rr (INC: r <1= rr) (CIH: forall (x: X), rr (x0 x)),
            forall (x: X), paco1 gf rr (x0 x)):
  forall (x: X), paco1 gf r (x0 x).
Proof.
  eapply paco_acc.
  intros. apply uncurry_adjoint1_1 in INC.
  eapply paco_mon.
  - apply (OBG (curry1 rr) INC CIH x).
  - intros. destruct x1. assumption.
Qed.
