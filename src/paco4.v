...
Context `{T3 : forall (x0: @T0) (x1: @T1 x0) (x2: @T2 x0 x1), Type}.

Local Open Scope paco_scope.
Local Notation rel := (rel4 T0 T1 T2 T3).
Local Notation "r <= r'" := (r <4= r') : paco_scope.
Local Notation "r <1= r'" := (r <5= r') : paco_scope.
Local Notation "r \/ r'" := (r \4/ r') : paco_scope.
Local Notation "r \1/ r'" := (r \5/ r') : paco_scope.

(** ** Predicates of Arity 4 *)

Let t : arityn 4 := Eval compute in (
    aritynS (@T0) (fun x0 =>
...
Inductive paco4 (gf : rel -> rel) (r : rel)  x0 x1 x2 x3 : Prop :=
  internal_mk_paco (_ : _paco gf r x0 x1 x2 x3).

Definition upaco4 (gf : rel -> rel) (r : rel) : rel := paco4 gf r \/ r.
Arguments paco4 : clear implicits.
Arguments upaco4 : clear implicits.
Hint Unfold upaco4 : core.

Local Notation bot := bot4.
Local Notation paco := paco4.
Local Notation upaco := upaco4.
...
Definition inst (gf : rel->_) r x0 x1 x2 x3 : paco_class (paco gf r x0 x1 x2 x3) := {|
...
Notation paco4 := paco4.paco4.
Notation upaco4 := paco4.upaco4.
Notation monotone4 := paco4.monotone.
